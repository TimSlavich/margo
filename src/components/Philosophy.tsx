...
const Philosophy = () => {
  ...
  const mobileText = (
    <>
      ...
      <blockquote className="hero-name quote-sentence-case text-center" style={{ color: 'var(--milk)', opacity: 0.9, fontSize: 'clamp(0.62rem, 1.8vw, 0.78rem)', marginTop: '4rem', paddingBottom: '0.5rem', width: '100%', maxWidth: 'min(38rem, 90%)', marginInline: 'auto', wordBreak: 'break-word' }}>
        "{t('philosophy.quote').split('. ').map((part, i) => (
          <span key={i}>{i > 0 && <br />}{part}{i === 0 ? '.' : ''}</span>
        ))}"
      </blockquote>
  ...
  return (
    <section id="philosophy" style={{ backgroundColor: '#3a171a' }}>
      ...
      {/* ── DESKTOP ── */}
      <div className="hidden desk:grid" style={{ gridTemplateColumns: '1fr 1fr', height: 'var(--real-100vh, 100svh)', width: '100%' }}>
        <div style={{ position: 'sticky', top: 0, height: 'var(--real-100vh, 100svh)', width: '100%', display: 'flex', flexDirection: 'column', alignItems: 'stretch', justifyContent: 'flex-start', padding: '3rem 3rem 4rem', overflow: 'hidden', minWidth: 0 }}>
          <p className="luxury-body text-center text-8xl uppercase leading-tight whitespace-nowrap px-1" style={{ color: 'var(--milk)', opacity: 0.7, letterSpacing: '0.02em', transform: 'scaleY(1.2)', transformOrigin: 'center', marginTop: '0', marginBottom: '5rem', flexShrink: 0 }}>
            {t('philosophy.label')}
          </p>
          {desktopContent}
        </div>
      ...
      <div className="desk:hidden">
        <div
          className="pt-[max(env(safe-area-inset-top),3.5rem)] px-4 pb-16"
